...
object UpdateManager {
    ...
    private const val UPDATE_URL = "https://your-project-name.deno.dev/check"

    fun checkUpdate(context: Context, onNewVersion: (versionName: String, changelog: String, url: String) -> Unit) {
            ...
            override fun onResponse(call: Call, response: Response) {
                response.body?.string()?.let { jsonString ->
                    try {
                        ...
                        // 比较版本号 (BuildConfig.VERSION_CODE)
                        if (serverVersionCode > BuildConfig.VERSION_CODE) {
                            onNewVersion(serverVersionName, changelog, downloadUrl)
                        }
                    } catch (e: Exception) {
